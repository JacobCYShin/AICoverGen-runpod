...
## RunPod Serverless with S3 URL Support

이 프로젝트는 RunPod Serverless 환경에서 S3 URL 방식을 지원하여 대용량 오디오 파일을 효율적으로 처리할 수 있습니다.

### S3 URL 방식의 장점
- **대용량 파일 지원**: base64 인코딩 방식보다 효율적으로 큰 파일 처리 가능
- **빠른 전송**: S3 URL을 통한 직접 다운로드로 전송 속도 향상
- **안정성**: 네트워크 타임아웃 및 메모리 문제 최소화

### 환경 설정

`.env` 파일을 생성하고 AWS S3 및 RunPod 설정을 추가하세요:
...
# base64 방식 사용 (레거시)
python test_runpod_aicovergen.py -k YOUR_API_KEY -e YOUR_ENDPOINT_ID --use-base64

# 출력 방식 지정
python test_runpod_aicovergen.py -k YOUR_API_KEY -e YOUR_ENDPOINT_ID --return-type url
```

### API 사용 예시

S3 URL 방식으로 API를 호출할 때:

```python
# S3에 먼저 오디오 파일들을 업로드
voice_url = upload_to_s3("vocals.wav")
instrument_url = upload_to_s3("instrumental.wav")
...
payload = {
    "input": {
        ...
        "params": {
            ...
            "voice_model": "Jungkook",
            "return_type": "url"  # S3 URL로 결과 반환
        }
    }
}
```
